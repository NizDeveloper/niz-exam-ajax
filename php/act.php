<?php
    ...
    while ($reg = $personas -> fetch_array()) {
      ...
      if ($nivelHuerfanismo == $filter) {
        echo "<tr>";
          ...
          echo "<td> <a href=\"#\" id=\"id_per=$reg[id_per]\" class=\"borrar\" data-bs-target=\"#alert\" data-bs-toggle=\"modal\">Borrar</a> </td>";
        echo "</tr>";
      } 
    }
  }
  ...
  if ($filter == 10) {
    pagination($mysql, $page);
  }

  function pagination($mysql, $page) {
    $personas = mysqli_query($mysql , 
    "select 
    per1.id_per as id_per,
    per1.nombres as nombres,
